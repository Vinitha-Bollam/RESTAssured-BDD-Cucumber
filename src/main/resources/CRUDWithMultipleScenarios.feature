@All @CRUD
Feature: Perform CRUD operations on Heroku booking app through multiple scenarios
  ...
  Scenario: Get the booking details after Updating the booking
    When I send a "GET" request
    Then the response is present
    And the response code is 200
    And the updated response has firstname as "Mahat"
  ...
  Scenario: Get the booking details for modified booking
    When I send a "GET" request
    Then the response is present
    And the response code is 200
    And the updated response has firstname as "Kunal"
